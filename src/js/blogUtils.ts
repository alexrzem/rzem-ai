 ...
 * ```
 */
export async function getAllPosts(lang?: (typeof locales)[number]): Promise<CollectionEntry<'blog'>[]> {
...
export function formatPosts(
    ...
    { filterOutFuturePosts = true, sortByDate = true, limit = undefined, removeLocale = true }: FormatPostsOptions = {},
    ...
    if (sortByDate) {
        filteredPosts.sort((a: CollectionEntry<'blog'>, b: CollectionEntry<'blog'>) => new Date(b.data.pubDate).getTime() - new Date(a.data.pubDate).getTime());
    } else {
 ...
 */
export function arePostsRelated(postOne: CollectionEntry<'blog'>, postTwo: CollectionEntry<'blog'>): boolean {
    ...
    if (postOne.data.categories && postTwo.data.categories && postOne.data.categories.length > 0 && postTwo.data.categories.length > 0) {
        const postOneCategories = postOne.data.categories.filter((category): category is string => typeof category === 'string').map((category) => slugify(category));

        const postTwoCategories = postTwo.data.categories.filter((category): category is string => typeof category === 'string').map((category) => slugify(category));
    ...
    let tagsMatch = false;
    if (postOne.data.tags && postTwo.data.tags && postOne.data.tags.length > 0 && postTwo.data.tags.length > 0) {
        const postOneTags = postOne.data.tags.filter((tag): tag is string => typeof tag === 'string').map((tag) => slugify(tag));

        const postTwoTags = postTwo.data.tags.filter((tag): tag is string => typeof tag === 'string').map((tag) => slugify(tag));
