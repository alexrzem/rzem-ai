...
import { dataTranslations, localizedCollections, routeTranslations, textTranslations } from '@/config/translationData.json';
 ...
 * ```
 */
export function getTranslatedData<T extends Locale, K extends DataKey<T>>(data: K, locale: T): (typeof dataTranslations)[T][K] {
 ...
 */
export function getLocalizedRoute(locale: (typeof locales)[number], baseRoute: string = '/', options?: { baseLocale?: (typeof locales)[number] }): string {
 ...
 */
export async function getLocalizedPathname(locale: (typeof locales)[number], url: URL): Promise<string> {
    ...
    if (routeTranslationsKey) {
        ...
        if (foundWildcardKey) {
            ...
            if (typeof mappedBase === 'string' && mappedBase.endsWith('/*')) {
                translatedRoute = mappedBase.slice(0, -2).replace(/\/$/, '') + (subPath ? '/' + subPath.replace(/^\//, '') : '');
            } else {
