...
const applyClasses = (el: AOSElement, top: number) => {
    ...
    if (options?.mirror && position?.out !== undefined && position.out !== false && top >= position.out && !options?.once) {
        hide();
 ...
 */
const handleScroll = (elements: AOSElement[]) => elements.forEach((el) => applyClasses(el, window.scrollY));
